...
namespace
{
	...
	void get_file(fileserver::unknown_digest const &requested_digest)
	{
		...
		Si::asio::connecting_observable connector(
		    socket, boost::asio::ip::tcp::endpoint(boost::asio::ip::address_v4::loopback(), 8080));
		auto connecting = Si::make_total_consumer(Si::make_coroutine_generator<Si::nothing>(
		    [&connector, &socket, &requested_digest](Si::push_context<Si::nothing> &yield) -> void
		    {
			    {
...

			    {
				    ...
				    Si::http::generate_request(
				        send_sink, make_get_request("localhost", "/" + fileserver::format_digest<Si::noexcept_string>(
				                                                           requested_digest)));
				    auto sending = Si::asio::make_writing_observable(socket);
				    sending.set_buffer(
				        Si::make_memory_range(send_buffer.data(), send_buffer.data() + send_buffer.size()));
...

			    {
				    ...
				    auto socket_source = Si::virtualize_source(Si::make_observable_source(
				        Si::asio::make_reading_observable(
...
int main(int argc, char **argv)
{
	...
	else if (verb == "clone")
	{
		...
		auto all = Si::for_each(fileserver::clone_directory(*requested, mount_point_manipulator, service, io),
		                        [&rc](boost::system::error_code ec)
		                        {
			                        if (ec)
			                        {
